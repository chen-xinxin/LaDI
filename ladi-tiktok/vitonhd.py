...
class VitonHDDataset(data.Dataset):
    def __init__(self,
        ...
        filename1 = os.path.join(dataroot, "test_unpairs.txt")
        
        with open(filename1, 'r') as f1:
            for line1 in f1.readlines():
                if order == 'unpaired':
                    im_name, c_name = line1.strip().split()
                    im_names.append(im_name)
                    c_names.append(c_name)
    ...
    def __getitem__(self, index):
        ...
        if "warped_cloth" in self.outputlist:  # Precomputed warped clothing image
            if self.order == 'unpaired':
                warped_cloth = Image.open(
                    ...
                    os.path.join(PROJECT_ROOT, 'data', 'warped_cloths_unpaired', 'vitonhd', category,
                                  im_name.split('/')[-1].replace(".png", "") + "_" + c_name))
        ...
        if "skeleton" in self.outputlist:
            ...
            skeleton = Image.open(
                ...
                os.path.join(dataroot, self.phase, 'openpose_img', im_name.replace('.png', '_rendered.png')))
        ...
        if "im_pose" in self.outputlist or "parser_mask" in self.outputlist or "im_mask" in self.outputlist or "parse_mask_total" in self.outputlist or "parse_array" in self.outputlist or "pose_map" in self.outputlist or "parse_array" in self.outputlist or "shape" in self.outputlist or "im_head" in self.outputlist:
            ...
            with open(os.path.join(dataroot, self.phase, 'openpose_json', pose_name), 'r') as f:
                ...
                pose_data[:, 0] = pose_data[:, 0] * (self.width / 604)
                pose_data[:, 1] = pose_data[:, 1] * (self.height / 1080)
            ...
            r = self.radius * (self.height / 1080.0)
            # r = self.radius * (self.height / 256.0)
            ...
            with open(os.path.join(dataroot, self.phase, 'openpose_json', pose_name), 'r') as f:
                ...
                data[:, 0] = data[:, 0] * (self.width / 604)
                data[:, 1] = data[:, 1] * (self.height / 1080)
                ...
                # ARM_LINE_WIDTH = int(90 / 256 * self.height)
...
if __name__ == '__main__':
    ...
    train_dataset = VitonHDDataset(phase='train',
                                       outputlist = ['c_name', 'im_name', 'cloth', 'image', 'im_cloth', 'im_mask', 'pose_map', 'category'],
                                       dataroot_path='tiktok',
                                       size=(512,384))
    ...
    # pose = p['pose_map'].permute(1,2,0).numpy()
    # pose *=255
    # pose = pose.astype(np.uint8)
    # pose= Image.fromarray(pose)
    # pose.save('pose_map.jpg')
