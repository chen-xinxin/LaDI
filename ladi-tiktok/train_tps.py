...
def extract_images(dataloader: DataLoader, tps: ConvNet_TPS, refinement: UNetVanilla, save_path: str, height: int = 512,
    ...
    for step, inputs in enumerate(tqdm(dataloader)):
        ...
        for cname, iname, warpclo, cat in zip(c_name, im_name, warped_cloth, category):
            ...
            #            quality=95)
            save_image(warpclo, os.path.join(save_path, cat, iname.split('/')[-1].replace(".png", "") + "_" + cname),
                       quality=95)
...
def parse_args():
    ...
    parser.add_argument('--wandb_project', type=str, default="tiktok_tps", help='wandb project name')
    parser.add_argument('--wandb_entity', type=str,default='chenxinxin', help='wandb entity name')
    ...
    parser.add_argument('--epochs_tps', type=int, default=25, help='number of epochs to train the TPS network')
    parser.add_argument('--epochs_refinement', type=int, default=25,
